...
class CocoDetectionEvaluator(object_detection_evaluation.DetectionEvaluator):
  ...
  def dump_detections_to_json_file(self, json_output_path):
    ...
    if json_output_path and json_output_path is not None:
      ...
      with tf.gfile.GFile(json_output_path.replace("predictions", "ground_truth"), 'w') as fid:
        tf.logging.info('Dumping gt to output json file.')
        json_utils.Dump(
            obj=self._groundtruth_list, fid=fid, float_digits=4, indent=2)
  ...
  def evaluate(self):
    ...
    try:
        print("DUMPING TO : " + os.getenv("JSON_OUT"))
        self.dump_detections_to_json_file(os.getenv("JSON_OUT"))
    except:
        print("environnement variable JSON_OUT is not defined, not dumping")
    return box_metrics
  ...
  def get_estimator_eval_metric_ops(self, eval_dict):
    ...
    print("r" * 200)
    print(self._include_metrics_per_category)
...
class CocoMaskEvaluator(object_detection_evaluation.DetectionEvaluator):
  ...
  def __init__(self, categories, include_metrics_per_category=False):
    ...
    self._annotation_id = 1
    # self._include_metrics_per_category = include_metrics_per_category
    self._include_metrics_per_category = True
