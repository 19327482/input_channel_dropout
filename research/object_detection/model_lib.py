...
def create_model_fn(detection_model_fn, configs, hparams, use_tpu=False,
                    postprocess_on_cpu=False, checkpoint_was_trained_on_rgb_only=False):
      ...
      if train_config.fine_tune_checkpoint and hparams.load_pretrained:
        ...
        available_var_map = (
            variables_helper.get_variables_available_in_checkpoint(
                ...
                checkpoint_was_trained_on_rgb_only=checkpoint_was_trained_on_rgb_only))
        if use_tpu:
          ...
          # original_first_layer_weights = [variable for variable in all_variables if variable.name == original_first_layer_weights_var_name][0]
          # print(original_first_layer_weights)
          # new_first_layer_weights_var_name = "FirstStageFeatureExtractor/resnet_v1_50/conv1/weights:0"
          # new_first_layer_weights = [variable for variable in all_variables if variable.name == new_first_layer_weights_var_name][0]
          # print(new_first_layer_weights)
          # sess.run(original_first_layer_weights.initializer)
          # print(original_first_layer_weights)
          # sess.run(new_first_layer_weights.initializer)
          # print(new_first_layer_weights)
          # channels_mean = tf.reduce_mean(original_first_layer_weights, axis=2)
          # channels_mean = tf.expand_dims(channels_mean, axis=2) * 3. / 13.
          # new_first_layer_weights = tf.concat([original_first_layer_weights * 3. / 13.] + [channels_mean] * 10, axis=2)
          # new_weights = sess.run(new_first_layer_weights)
          # print(new_weights)
          # print(type(new_weights))
          # print(new_weights.shape)
          # new_first_layer_weights.se
          # new_first_layer_weights.load(new_weights)

          # original_first_layer_weights = original_first_layer_weights * 3. / 13.
          # for _ in range(10):
          #    original_first_layer_weights = tf.concat([original_first_layer_weights, channels_mean], axis=2)
...
def create_estimator_and_inputs(run_config,
  ...
  model_fn = model_fn_creator(detection_model_fn, configs, hparams, use_tpu,
                              postprocess_on_cpu, checkpoint_was_trained_on_rgb_only)
...
def create_train_and_eval_specs(train_input_fn,
  ...
  train_spec = tf.estimator.TrainSpec(
      input_fn=train_input_fn, max_steps=train_steps, hooks=train_hooks)
